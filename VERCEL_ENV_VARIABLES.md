...
Copia estas variables directamente en Vercel Dashboard → Settings → Environment Variables.
...
# Cron Security (opcional pero recomendado)
...
- **NUNCA** expongas variables privadas (sin `NEXT_PUBLIC_`) en el frontend
- **Actualiza** `NEXT_PUBLIC_APP_URL` con tu URL real de Vercel después del deploy
- **Genera** un `CRON_SECRET` único y seguro
...
Después del deploy, verifica que las variables estén cargadas:
...
Si las variables están correctas, deberías ver respuestas exitosas.
