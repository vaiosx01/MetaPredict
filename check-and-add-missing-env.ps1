# Script para verificar y agregar variables de entorno faltantes en Vercel
# Uso: .\check-and-add-missing-env.ps1
...
# Obtener lista de variables actuales en Vercel
Write-Host "📋 Obteniendo variables actuales..." -ForegroundColor Yellow
$currentVars = vercel env ls --json 2>&1 | ConvertFrom-Json

if (-not $currentVars) {
    Write-Host "⚠️  No se pudieron obtener las variables actuales. Continuando con inyección completa..." -ForegroundColor Yellow
    $currentVarNames = @()
} else {
    $currentVarNames = $currentVars | ForEach-Object { $_.key } | Sort-Object -Unique
    Write-Host "✅ Encontradas $($currentVarNames.Count) variables actuales" -ForegroundColor Green
}

# Variables requeridas del env.example
$requiredVars = @{
    # Frontend - Públicas (todas las entornos)
    ...
    # Backend - Privadas (solo production)
...
foreach ($key in $requiredVars.Keys) {
    if ($currentVarNames -notcontains $key) {
        $missingVars[$key] = $requiredVars[$key]
    }
}

if ($missingVars.Count -eq 0) {
    Write-Host "`n✅ Todas las variables están configuradas!" -ForegroundColor Green
    Write-Host "Total de variables: $($requiredVars.Count)" -ForegroundColor Cyan
    exit 0
}

Write-Host "`n📊 Resumen:" -ForegroundColor Cyan
Write-Host "  - Variables requeridas: $($requiredVars.Count)" -ForegroundColor White
Write-Host "  - Variables actuales: $($currentVarNames.Count)" -ForegroundColor White
Write-Host "  - Variables faltantes: $($missingVars.Count)" -ForegroundColor Yellow

Write-Host "`n🔐 Agregando variables faltantes..." -ForegroundColor Cyan
Write-Host ""
...
foreach ($key in $missingVars.Keys) {
    $value = $missingVars[$key]
    $count++
    
    Write-Host "[$count/$($missingVars.Count)] Agregando $key..." -ForegroundColor Gray -NoNewline
    
    # Determinar entornos (formato: espacios, no comas)
    ...
    # Agregar la variable
    $result = Write-Output $value | vercel env add $key $envs 2>&1
...
Write-Host ""
Write-Host "✅ Variables agregadas exitosamente: $success" -ForegroundColor Green
if ($errors -gt 0) {
