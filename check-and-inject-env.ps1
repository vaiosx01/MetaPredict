...
# Variables que necesitamos (todas las necesarias)
$requiredVars = @{
    # Frontend - Públicas
...
# Crear hash de variables actuales
$currentVarNames = @{}
if ($currentVars) {
    foreach ($var in $currentVars) {
        $currentVarNames[$var.key] = $true
    }
}
...
foreach ($key in $requiredVars.Keys) {
    if (-not $currentVarNames.ContainsKey($key)) {
...
Write-Host "  - Variables faltantes: $($missingVars.Count)" -ForegroundColor $(if ($missingVars.Count -eq 0) { "Green" } else { "Yellow" })
Write-Host ""
...
Write-Host "🔧 Agregando $($missingVars.Count) variables faltantes..." -ForegroundColor Yellow
Write-Host ""
